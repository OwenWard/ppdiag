...
#' print a raw raw plot for fitting single poisson process to a network
#'
#' 
...
#' @return a raw residual plot for fitting a single poisson process to a network
#' @export



network_rawresidual.hpp  <- function(t,n_node,lambda0, termination,start_time,end_time){
  ...
  for (i in 1:nrow(t)){
    ...
    mat[sender,receiver] <- rawresidual.hpp(object, time, termination)
  }
