test_that("test pp_simulate", {
  ...
  ## tests for hpp
  expect_message(pp_simulate(hpp(lambda = 1), start = 2,end = 3, n = 5),
                 "5 events simulated. To simulate up to an endtime set n=NULL.")
  expect_message(pp_simulate(hpp(lambda = 1), start = 2, end = 3),
                 "Simulating up to endtime. To simulate n events specify n.")
  expect_message(pp_simulate(hpp(lambda = 1, events = c(1)),
                             start = 2, 
                             end = 3),
                 "Events in the hpp object will be overwritten by simulated events.")
  expect_null(pp_simulate(hpp(lambda = 1), start = 0, end = -1))
  expect_null(pp_simulate(hpp(lambda = 1), start = 0, end = 0))
  expect_error(pp_simulate(hpp(lambda = 1), n = 0),
               "n must be positive for simulation")
  expect_error(pp_simulate(hpp(lambda = 1), n = -2),
               "n must be positive for simulation")
  expect_error(pp_simulate(hpp(lambda = 1), start = 0, end = NULL),
  ...
  expect_error(pp_simulate(mmhp(lambda0 = 0.9, lambda1 = 1.1,
                                 alpha = 1, beta = 0.5)),
  ...
  expect_message(pp_simulate(mmhp(lambda0 = 0.9, lambda1 = 1.1,
                                alpha = 0.1, beta = 0.5, delta = c(0.5, 0.5),
