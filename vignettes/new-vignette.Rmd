...
univariate temporal point 
processes. 

Existing functions aim at: 

- Simulating data from a range of common univariate point processes including 
Homogeneous Poisson Process, Hawkes Process, and Markov-modulated Hawkes 
Process.
...
For demonstration of how this package would help users with diagnostics of 
models, we apply our functions to the following Brastings dataset from the SAPP 
package: 
...
In the network setting, Pearson residual is more appropriate in measuring the models than raw residuals.[@Jing2020] The diagnostics of the HPP model shows pearson residual=-0.894, and the diagnostics of the HP model shows pearson residual=1.56. Both models seem to produce similar absolute residuals. We then look at the k-s statistics, for HPP, the D-statistic is 0.124 with p-value=0.000212; for HP, the D-statistic is 0.0416 with p-value=0.680. This indicates that HP is a better fit of the intensity with its D-statistic closer to 0 and a p-value closer to 1. So diagpp helps us to understand that the email data is better fitted to the Hawkes model in comparison with the Homogeneous Poisson model. 
