...
`fithp(events)` returns an object of class `hp`, estimating the
three parameters of the Hawkes process using `constrOptim`, to
ensure the solution is feasible. This can be a challenging optimisation problem
...
`pp_diag(object, events)` gives diagnostics of the model,
...
`pp_residual(object, events, start = min(events),end = max(events))` gives raw and pearson residuals. 
