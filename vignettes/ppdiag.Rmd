...
## Simulating data
Functions for simulating data include `simulatehpp`, `simulatehp`,
and `simulatemmhp`, which take in objects of type `hpp`, `hp`, and `mmhp`,
respectively, with additional parameters for simulation, and produces a vector
of simulated event times. 

- Homogeneous Poisson Process: 

`simulatehpp(hpp)` takes a hpp object and returns a vector of event times
...
```{r}
hpp_events <- simulatehpp(hpp_obj, end=10)
hpp_events
```

Or you can specify a vector of certain length.
```{r}
simulatehpp(hpp_obj, end=10, n=50)
```

- Hawkes Process: 

`simulatehp(hp, start = 0, end, history = 0, n=NULL)` returns event times
...
given these events.
```{r}
hp_events <- simulatehp(hp_obj, start = 0, end = 50)
hp_events
```

You can also specify desired length of events. 
```{r}
hp_events <- simulatehp(hp_obj, start = 0, end = 50, n=10)
hp_events
```

- Markov-Modulated Hawkes Process: 

`simulatemmhp(mmhp, n = 1, ...)` 
...
```{r}
init <- rep(0.1,3)
fit_hp <- fithp(init, events=hp_events$t)
fit_hp
```
...
`drawHPPIntensity(hpp, events, color = "red", start = 0, end = max(events), fit=FALSE, 
                             int_title = "Intensity homogeneous Poisson Process")` plots the intensity of a homogeneous poisson process.
```{r,fig.width=4,fig.height=4}
drawHPPIntensity(fit_hpp, events=hpp_events, color = "red", fit=FALSE, 
                             int_title = "Intensity homogeneous Poisson Process")
```

To plot the fitted intensity on the inputted events, specify fit=TRUE.
```{r,fig.width=4,fig.height=4}
drawHPPIntensity(fit_hpp, events=hp_events$t,
                 color = "red", fit=TRUE)
```

`drawHPIntensity(hp, start = 0, end = max(events), history=0, events,
                            color = 1, i = 1, add=FALSE, fit=FALSE, vec=rep(0.1,3), 
                            int_title="Hawkes Intensity")`
plots the intensity of a hawkes process.
```{r,fig.width=4,fig.height=5}
drawHPIntensity(fit_hp, start = min(hp_events$t), end = max(hp_events$t),
               history = 0, events = hp_events$t)
```

To plot the fitted intensity on the inputted events, specify fit=TRUE.
```{r,fig.width=4,fig.height=5}
drawHPIntensity(fit_hp, start = min(hp_events$t), end = max(hp_events$t),
                history = 0, events = hp_events$t, fit=TRUE)
```

`drawUniMMHPIntensity(mmhp, simulation, add = FALSE, color = 1, fit = FALSE, 
                                 given_main = "Intensity Plot of MMHP")` plots the intensity of a 
mmhp process. 
```{r,fig.width=4,fig.height=5}
...
```{r,fig.width=4,fig.height=4}
diagpp(hp_obj,hp_events$t)
```

`rawresidual(object, events, start = min(events),end = max(events))` gives rawresidual only. 

```{r}
rawresidual(hp_obj,hp_events$t)
```

`pearsonresidual(object, events, start = min(events),end = max(events))` gives pearsonresidual only. 

```{r}
pearsonresidual(hp_obj,hp_events$t)
```

`intensityqqplot(object, events, pzt=NULL)` gives both qqplot and intensity plot. 

```{r,fig.width=7,fig.height=4}
intensityqqplot(hp_obj, hp_events$t)
```
