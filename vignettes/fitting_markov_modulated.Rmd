...
Having simulated this data we compile the Stan model and draw posterior 
samples.
...
```{r mmpp-plot}
bayesplot::mcmc_hist(fit_mmpp$draws(), 
                     pars = c("lambda0", "c"))
```
...
```{r mmpp-extract-post-mean}
mmpp_post <- fit_mmpp$summary() %>%
...
# use the mmpp data here for now
...
```{r plot-draws-mmhp}
bayesplot::mcmc_hist(fit_mmhp$draws(),
...
```{r rstan-mmpp, eval=FALSE}
mmpp_rstan <- stan(file = mmpp_file, 
                   data = mmpp_data)
