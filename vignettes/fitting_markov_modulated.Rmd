...
mmpp_data <- list(num_events = length(sim$events) - 1, 
                  # as first event is start time (0)
                  time_matrix = diff(c(sim$events, sim$end))
...
of each of the parameters. This can be achieved using either the
posterior mean or posterior median.
...
mmhp_data <- list(num_events = length(sim$events) - 1, 
                  # as first event is start time (0)
                  time_matrix = diff(c(sim$events, sim$end))
                  #interevent arrival time
                  )
